...
const GenreFilms: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [genreRes, filmsRes] = await Promise.all([
          client.get(`/genres/${id}`),
          client.get(`/genres/${id}/films`)
        ]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {films.length === 0 ? (
        <p className="text-gray-500 dark:text-gray-400">No films found in this genre.</p>
      ) : (
